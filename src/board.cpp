...
namespace Board {
    ...
    std::map<char, PieceTypes> char_piece_map {
        {'p', PieceTypes::Pawn},
        {'b', PieceTypes::Bishop},
        {'n', PieceTypes::Knight},
        {'r', PieceTypes::Rook},
    ...
    // just don't look here please, it works forget about it
    void Board::set_fen(const char *fen_string){
        // bb_it keeps track of board cell index of it
        int bb_it = 56;
        const char *it = fen_string;
        ...
        while (*it != ' ') {
            switch (*it) {
            case 'K': {
                single_update_castle_rights(1, Color::White, 1);
                break;
            }
            case 'Q': {
                single_update_castle_rights(1, Color::White, 0);
                break;
            }
            case 'k': {
                single_update_castle_rights(1, Color::Black, 1);
                break;
           }
            case 'q': {
                single_update_castle_rights(1, Color::Black, 0);
                break;
            }
            case '-': {
                it++;
    ...
    void Board::set_square_fen(char c, int &sq) {
        int color = !isupper(c);
        PieceTypes type = char_piece_map[tolower(c)];
    ...
    void Board::put_piece(int sq, PieceTypes type, int color) {
        ...
        hash_key ^= prns_pieces[int(type)][color][sq];
    }
    ...
    MoveType Board::get_type(Move &move) {
        if (move.move_type == MoveType::Promotion || move.move_type == MoveType::Capture)
            return move.move_type;
        if (move.piece == PieceTypes::Pawn && abs(int(move.from) - int(move.to)) == 16) {
            return MoveType::Double_Pawn_Push;
        } else if (move.piece == PieceTypes::King &&
                   (abs(int(move.from) - int(move.to)) > 1 && abs(int(move.from) - int(move.to)) < 5)) {
    ...
    void Board::make_move(Move &move) {
        ...
        change_color();
#ifdef ITERATIVE_EVAL
        eval.update(move, color_to_move, false);
#endif
    }
    ...
    void Board::single_update_castle_rights(bool value, Color color, bool left) {
        // if castle state was changed
        if (irr_data.castle_rights[int(color)][left] != value)
            hash_key ^= prns_castle[int(color) * 2 + left];
        irr_data.castle_rights[int(color)][left] = value;
    }

    void Board::unmake_move(Move &move) {
        change_color();
#ifdef ITERATIVE_EVAL
        eval.update(move, color_to_move, true);
#endif
    ...
    Moves Board::generate_pl_moves(bool castles) {
        ...
        generate_rook_actions(bb_piece_type[int(color_to_move)][int(PieceTypes::Rook)], bb_occ_rot, color_to_move, bb_occ,
                              moves);
        generate_bishop_actions(bb_piece_type[int(color_to_move)][int(PieceTypes::Bishop)], bb_occ_rot_main_diag,
                                bb_occ_rot_second_diag, color_to_move, bb_occ[int(color_to_move)], moves);
        generate_queen_actions(bb_piece_type[int(color_to_move)][int(PieceTypes::Queen)], bb_occ_rot, bb_occ_rot_main_diag,
                               bb_occ_rot_second_diag, color_to_move, bb_occ, moves);
        bb_occ[int(color_to_move)] &= ~bb_piece_type[int(NOT_COLOR(color_to_move))][int(PieceTypes::King)];
    ...
    void Board::generate_castle_moves(Moves &moves) {
        // possible to get rid of the checking of the final king square to get pseudo
        // legal moves instead of legal
        if (color_to_move == Color::Black) {
            if (irr_data.castle_rights[1][CASTLE_LEFT] == 1)
                if (is_empty(bb_occ[2], {Square::b8, Square::c8, Square::d8}) &&
                    !(is_attacking(Square::b8, color_to_move, false) && is_attacking(Square::c8, color_to_move, false) &&
                      is_attacking(Square::d8, color_to_move, false) && is_attacking(Square::e8, color_to_move, true)))
            ...
            if (irr_data.castle_rights[1][CASTLE_RIGHT] == 1)
                if (is_empty(bb_occ[2], {Square::g8, Square::f8}) &&
                    !(is_attacking(Square::f8, color_to_move, false) && is_attacking(Square::g8, color_to_move, false) &&
            ...
            if (irr_data.castle_rights[0][CASTLE_LEFT] == 1)
                if (is_empty(bb_occ[2], {Square::b1, Square::c1, Square::d1}) &&
                    !(is_attacking(Square::b1, color_to_move, false) && is_attacking(Square::c1, color_to_move, false) &&
                      is_attacking(Square::d1, color_to_move, false) && is_attacking(Square::e1, color_to_move, true)))
            ...
            if (irr_data.castle_rights[0][CASTLE_RIGHT] == 1)
                if (is_empty(bb_occ[2], {Square::g1, Square::f1}) &&
                    !(is_attacking(Square::f1, color_to_move, false) && is_attacking(Square::g1, color_to_move, false) &&
    ...
    bool Board::is_attacking(Square sq, Color sq_color, bool is_king) {
        ...
        if ((get_first_diagonal_attack(sq, bb_occ_rot_main_diag) |
             get_second_diagonal_attack(sq, bb_occ_rot_second_diag)) &
            (bb_piece_type[int(NOT_COLOR(sq_color))][int(PieceTypes::Bishop)] |
             bb_piece_type[int(NOT_COLOR(sq_color))][int(PieceTypes::Queen)])) {
            return true;
        }
        if ((get_file_attack(sq, bb_occ_rot) | get_rank_attack(sq, bb_occ[2])) &
            (bb_piece_type[int(NOT_COLOR(sq_color))][int(PieceTypes::Rook)] |
             bb_piece_type[int(NOT_COLOR(sq_color))][int(PieceTypes::Queen)])) {
            return true;
        }
    ...
    int evaluate(Board &board){
        ...
        for(int type = 0; type < 6; type++){
            PieceTypes p_type = static_cast<PieceTypes>(type);
            score += material_w[type] * (int)(board.piece_count(p_type, Color::White) - board.piece_count(p_type, Color::Black));
            score += apply_pst(board.bb_piece_type[int(Color::White)][type], pst_values[type], Color::White) - apply_pst(board.bb_piece_type[int(Color::Black)][type], pst_values[type], Color::Black);
        }
        return score * nm_score_sign(board.color_to_move); 
    }
